...
use LauLaman\ReceiptPrinter\Domain\Command\Layout\NewLine;
...
// ========================================================================================================================================================================
     ...
     */
    public function negative(?string $text = null): self
    {
        if ($text === null) {
            return $this->open(Negative::class);
        }
        $this->current()[] = new Negative(new Text($text));
        return $this;
    }
     ...
     */
    public function column(string|callable $left, string|callable $right, string|callable $short = null, ?int $indent = null, ?string $spacer = ' '): self
    {
        if (is_string($left)) {
            $left = [new Text($left)];
        } else {
        ...
        if (is_string($short)) {
            $short = [new Text($short)];
        } elseif ($short !== null) {
            $shortBuilder = self::create();
            $short($shortBuilder);
            $short = $shortBuilder->build()->getCommands();
        }

        $this->current()[] = new Column($left, $right, $short, $indent, $spacer);
